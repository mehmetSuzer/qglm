...
static inline Q_MAT4 orthographic(Q_TYPE width, Q_TYPE height, Q_TYPE near, Q_TYPE far)
{
    const Q_TYPE far_plus_near = q_add(far, near);
    ...
    const Q_TYPE zz = q_negate(q_mul(Q_TWO, inv_far_minus_near));
    const Q_TYPE zw = q_negate(q_mul(far_plus_near, inv_far_minus_near));
...
static inline Q_MAT4 perspective(Q_TYPE fovw, Q_TYPE aspect_ratio, Q_TYPE near, Q_TYPE far)
{
    ...
    const Q_TYPE zw = q_negate(q_mul(q_mul(q_mul(Q_TWO, far), near), inv_far_minus_near));
    const Q_TYPE wz = q_negate(Q_ONE);
